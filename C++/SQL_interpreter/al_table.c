...
static enum Errors GetCurrentRecord (struct Table_type * tabHandle);
static enum Errors PutCurrentRecord (struct Table_type * tabHandle);
static enum Errors PutNewRecord (struct Table_type * tabHandle,
        long position, struct links * links);
static enum Errors ModifyLinks (struct Table_type *  tabHandle,
        long position, long value, enum Direction dir);
static enum Errors GetInsertPos (struct Table_type * tabHandle, long * position);
static enum Errors GetLinks (struct Table_type * tabHandle,
        long position, struct links * links);
static enum Errors WriteHeader (struct Table_type * tabHandle);
static enum Errors ReadHeader (struct Table_type * tabHandle);
static struct Table_type * CreateTableHandle (struct TableStruct * tableStruct);
static struct Table_type * AllocateTableHandle (void);
static void DeleteTableHandle (struct Table_type * tabHandle);
static enum Errors AllocateBuffers (struct Table_type * tabHandle);
static void DeallocateBuffers (struct Table_type * tabHandle);
static enum Errors AddRecToDeleted (struct Table_type * tabHandle);
...
enum Errors getFieldName(THandle tableHandle, unsigned index, char ** pFieldName)
{
    ...
    *pFieldName = (char*)tableHandle->pFieldStruct[index].fieldName;
    return OK;
}
...
static struct Table_type * AllocateTableHandle (void)
{
    struct Table_type * handle;

    handle = (struct Table_type *) calloc (1, sizeof (struct Table_type));
